...
class Stat:
    @staticmethod
    def rdivDemo(data, isLatex=False, globalMinMax=False, high=100, low=0, higherTheBetter=False):
        if isLatex:
            ...
            if higherTheBetter:
                ranks = map(list, ranks)
                max_rank = max(zip(*ranks)[0])
                for i in range(len(ranks)):
                    ranks[i][0] = max_rank - ranks[i][0]
                ranks = map(tuple, ranks)
            ...
            for _, __, x in sorted(ranks):
                ...
                if higherTheBetter:
                    ...
                    rr = x.rank + 1
                print('%4s , %12s ,    %0.2f  ,  %0.2f ' % (rr, x.name, x.median(), x.spread())) +  xtile(x.all, lo=lo, hi=hi, width=30)
                last = x.rank
            return ranks
        ...
        for line in f.readlines():
            for word in line.split():
                if not word.replace('.','',1).isdigit():
                    ...
                    allData[P].append(float(word))

    Stat.rdivDemo([[k] + v for k, v in allData.items()], isLatex=latex, higherTheBetter=higherBetter)
